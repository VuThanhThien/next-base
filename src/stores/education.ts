...
const addEducation =
  ...
  ({ institution, studyType, area, startDate, isStudyingHere, endDate, id, url, score, courses }: IEducationItem) =>
    set(
...
const updateEducation = (set: SetState<IEducationStore>) => (index: number, updatedInfo: IEducationItem) => {
  set(
    produce((state: IEducationStore) => {
